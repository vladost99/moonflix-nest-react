  ...
  return (
    <Link to={mediaType !== 'people' ? routesGen.mediaDetail(mediaType, media.mediaId || media.id) : routesGen.person(media.id)}>
      ...
      >
        {mediaType !== 'people' && (
          <>
            {favoriteUtils.check({ listFavorites, mediaId: media.id || media.mediaId }) && (
              <Favorite
                color="primary"
                sx={{
                  position: 'absolute',
            ...
            >
              <Stack spacing={{ xs: 1, md: 2 }}>
                ...
                <Typography variant="body1" fontWeight="700" sx={{ fontSize: '1rem', ...uiConfigs.style.typoLines(1, 'left') }}>
                  {title}
                </Typography>
              </Stack>
            </Box>
