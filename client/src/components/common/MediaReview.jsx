...
const MediaReview = ({ reviews, media, mediaType }) => {
  ...
  const onAddReview = async () => {
    ...
    if (err) toast.error(Array.isArray(err.message) ? err.message.join(' ') : err.message);
    if (response) {
          ...
          <>
            ...
            <Stack direction="row" spacing={2}>
              ...
              <Stack spacing={2} flexGrow={1}>
                ...
                <TextField value={content} onChange={(e) => setContent(e.target.value)} multiline rows={4} placeholder="Write your review" variant="outlined" />
                <LoadingButton
                  variant="contained"
                  size="large"
